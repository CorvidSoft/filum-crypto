...
//! security baseline. This differs from the parent Filum DESIGN.md §4.2 which
...
///
/// Returns `Err(FilumCryptoError::Randomness)` if the OS entropy source is
...
pub fn phrase_to_secret(phrase: &str) -> Result<[u8; 32]> {
    let mnemonic = Mnemonic::parse_in(Language::English, phrase)
        .map_err(|_| FilumCryptoError::InvalidPhrase)?;
    let mut entropy = mnemonic.to_entropy();
    if entropy.len() != 32 {
        entropy.zeroize();
        return Err(FilumCryptoError::InvalidPhrase);
    }
    let mut out = [0u8; 32];
...
mod tests {
    ...
    fn invalid_phrase_rejected() {
        ...
        assert!(matches!(result, Err(FilumCryptoError::InvalidPhrase)));
    }
