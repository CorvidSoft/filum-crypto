#!/usr/bin/env bash
#
# Cross-compile filum-crypto-uniffi to the iOS targets, assemble an
...
XCFRAMEWORK="$BUILD_DIR/FilumCryptoFFI.xcframework"
XCFRAMEWORK_ZIP="$BUILD_DIR/FilumCryptoFFI.xcframework.zip"
...
cargo build --release --target aarch64-apple-ios --package filum-crypto-uniffi

echo "→ Building for aarch64-apple-ios-sim (Apple Silicon simulator)..."
cargo build --release --target aarch64-apple-ios-sim --package filum-crypto-uniffi

echo "→ Building for x86_64-apple-ios (Intel simulator)..."
cargo build --release --target x86_64-apple-ios --package filum-crypto-uniffi

echo "→ lipo simulator slices into a universal archive..."
# arm64 listed first so lipo reports "arm64 x86_64" in the fat header.
lipo -create \
    "$ROOT/target/aarch64-apple-ios-sim/release/libfilum_crypto.a" \
    "$ROOT/target/x86_64-apple-ios/release/libfilum_crypto.a" \
    -output "$SIM_UNIVERSAL_DIR/libfilum_crypto.a"

# Sanity check: both archs must be present (order in the fat header varies).
LIPO_INFO="$(lipo -info "$SIM_UNIVERSAL_DIR/libfilum_crypto.a")"
...
cp "$ROOT/Sources/FilumCrypto/filum_cryptoFFI.h"        "$HEADERS_DIR/filum_cryptoFFI.h"
cp "$ROOT/Sources/FilumCrypto/filum_cryptoFFI.modulemap" "$HEADERS_DIR/module.modulemap"
...
cargo run --quiet --package filum-crypto-uniffi --bin uniffi-bindgen -- \
    ...
    "$ROOT/target/aarch64-apple-ios/release/libfilum_crypto.a"

if ! diff -q "$TMP_BINDINGS/FilumCrypto.swift"        "$ROOT/Sources/FilumCrypto/FilumCrypto.swift" \
  || ! diff -q "$TMP_BINDINGS/filum_cryptoFFI.h"      "$ROOT/Sources/FilumCrypto/filum_cryptoFFI.h" \
  || ! diff -q "$TMP_BINDINGS/filum_cryptoFFI.modulemap" "$ROOT/Sources/FilumCrypto/filum_cryptoFFI.modulemap"; then
...
xcodebuild -create-xcframework \
    -library "$ROOT/target/aarch64-apple-ios/release/libfilum_crypto.a" -headers "$HEADERS_DIR" \
    -library "$SIM_UNIVERSAL_DIR/libfilum_crypto.a"                    -headers "$HEADERS_DIR" \
    -output "$XCFRAMEWORK"

echo "→ Zipping XCFramework..."
( cd "$BUILD_DIR" && zip -qr "FilumCryptoFFI.xcframework.zip" "FilumCryptoFFI.xcframework" )
